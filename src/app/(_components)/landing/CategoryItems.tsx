...
export default function CategoryItems({ category, type }: { category: string; type: 'search' | 'like' }) {
  ...
  return (
    <li className="flex flex-col items-center text-xs text-b3">
      <Link
        className="w-[50px] h-[48px] rounded-md border border-gray-300 bg-white mb-[3px] tracking-[-0.02em] flex justify-center items-center cursor-pointer"
        href={
          type === 'like'
            ? `/mypage/like/spot${category === '전체' ? '' : `?type=${item.href}`}`
            : `/category${category === '전체' ? '' : `?type=${item.href}`}`
        }
      >
