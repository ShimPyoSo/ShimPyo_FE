  ...
  return (
    <div
      className={`flex flex-col justify-center items-center ${
        type === 'search' ? 'h-[calc(100vh-220px)]' : 'h-[calc(100vh-300px)]'
      }`}
    >
      <Image className="lg:mt-0 mb-[24px]" src={noResult} alt="결과 없음" width={102} height={130} />
      ...
      <span className="mt-[8px] flex gap-[16px] tracking-[-0.02em] text-xs">
        <p className="text-gn1 font-semibold">이런 관광지는 어때요?</p>
        <ul className="text-g1 flex gap-[8px]">
          {['템플스테이', '요가', '촌캉스', '스파', '비건'].map((keyword: string) => (
            <li key={keyword}>
